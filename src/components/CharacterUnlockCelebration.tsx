...
export function CharacterUnlockCelebration({
  ...
  return (
    ...
    >
      <div
        className="rm-card max-w-sm p-8 text-center"
        onClick={(e) => e.stopPropagation()}
      >
        <p className="text-sm font-medium uppercase tracking-wide text-muted">New character</p>
        <span className="mt-4 block text-7xl">{character.avatar}</span>
        <h3 className="mt-4 text-2xl font-semibold text-foreground">{character.name}</h3>
